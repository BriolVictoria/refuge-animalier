<main class="w-full">
    @if (session()->has('success'))
        <div
            ...
            x-show="show"
            x-transition
            class="fixed top-10 right-10 z-50 px-4 py-3 rounded-lg bg-green-600 text-white text-md shadow-md"
        >
            {{ session('success') }}
        </div>
    @endif
    ...
    <div>
        <div class="flex items-center gap-2 border-b border-blue-900">
            <a title="{{ __('admin/animals.ariane.animals_index') }}" href="{!! route('animals.index', ['locale' => app()->getLocale()]) !!}"
               class="py-4 px-8 text-xs text-blue-900 font-light md:text-lg">
                {{ __('admin/animals.animals.title') }}
            </a>
            <img src="{!! asset('assets/img/arrow_arianne.svg') !!}" class="w-4 h-4" alt="{{ __('admin/animals.animals.image_alt') }}">
            <h1 class="py-4 px-8 text-sm text-blue-900 font-semibold md:text-xl">
                {{ __('admin/animals.ariane.animal_show', ['name' => $animal->name]) }}
            </h1>
        </div>
        ...
        <x-admin.animals.show
            ...
            label="{{ __('admin/animals.actions.add_note') }}"
            class="bg-blue-900 self-start text-white transition-all duration-300 hover:scale-101 hover:bg-blue-600"
        />
    </div>

    @if($openVisitNote)
        <x-admin.modal.modal title="{{ __('admin/animals.ariane.animals_index') }}">
            <x-admin.form.field.form wire="create">
    ...
    @if($noteToEdit)
        <x-admin.modal.modal title="{{ __('admin/animals.buttons.edit_note') }}">
            <x-admin.form.field.form wire="update">
                <x-admin.form.field.input
                    wire="noteEmail"
                    field_name="{{ __('admin/animals.fields.email') }}"
                    label="{{ __('admin/animals.fields.email') }}"
    ...
    @if($openModalForDeleteAnimal)
        <x-admin.modal.modal title="{{ __('admin/animals.delete_modal.animal.title') }}">
            ...
            <div class="flex flex-col justify-center gap-4 pt-4">
                <x-admin.button.delete_button
                    ...
                    class="px-6 py-2 bg-red-600 text-white text-lg rounded-lg transition-all duration-300 hover:bg-red-700 hover:scale-105"
                >
                    {{ __('admin/animals.buttons.delete') }}
                </x-admin.button.delete_button>

                <x-admin.button.button
                    wire:click="closeModalAnimal"
                    route_name="#"
                    title_button="{{ __('admin/animals.delete_modal.animal.cancel_button') }}"
                    label="{{ __('admin/animals.buttons.cancel') }}"
                    class="px-6 py-2 border border-gray-300 text-gray-600 rounded-lg transition-all duration-300 hover:bg-gray-100"
                />
            </div>
        </x-admin.modal.modal>
    @endif

    @if($openModalForDeleteNote)
        <x-admin.modal.modal title="{{ __('admin/animals.delete_modal.note.title') }}">
            <p class="text-sm text-gray-600">
